<?php
	namespace quizzenger\plugins\achievements {
		...
		class QuestionRatingAchievement implements IAchievement {
			public function grant(SqlHelper $database, UserEvent $event) {
				$database = $database->database();
				$userId = $event->user();
				$questionCount = $event->get('question-count');
				$minRating = $event->get('min-rating');
				$minRatingCount = $event->get('min-rating-count');

				$statement = $database->prepare('SELECT COUNT(*) as count FROM question '
						.' WHERE user_id=? AND rating >= ? AND ratingCount >= ?');

				$statement->bind_param('iii', $userId, $minRating, $minRatingCount);
		...
		} // class QuestionRatingAchievement
	} // namespace quizzenger\plugins\achievements
?>
