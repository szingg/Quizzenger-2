...
class FileUpload{
	...
	public function processFileUpload(){
		...
		if (!$validExtension || !$containsExtension){
			$this->sendResponse('error', 'Das Format der Datei ist ung�ltig.');
			return;
		}
		...
		if(!$validSize){
			$this->sendResponse('error', 'Die Datei ist zu gross. Maximale Gr�sse: '.MAX_ATTACHMENT_SIZE_KByte.' KByte');
			return;
		}
